...
export class SnakeGameEngine {
  ...
  animate(isPlaying: boolean) {
    ...
    if (this.currentFrameCount < this.staggerFrame) {
      ...
      if (this.isGameOver()) {
        this.setLives((prevLives) => {
          const newLives = prevLives - 1;
          if (newLives <= 0) {
            this.setIsGameOver(true); // Game is fully over
          } else {
            this.setIsGameOver(true); // Game over but respawning
          }
          return newLives;
        });
        return; // Stop further execution to allow respawn logic
      }
      ...
      if (this.snake.justAte) {
        ...
        if (newScore !== this.externalScore) {
          this.setScore(newScore); // Avoid redundant updates
        }
      }
    ...
    this.animationFrameId = requestAnimationFrame(() =>
      this.animate(this.internalPlayState)
    );
  }
